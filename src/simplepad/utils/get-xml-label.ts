export function getUserName(linkList: any, name: string) {
    const otherObjectArray = linkList.filter((link: any) =>
        name.includes(link.$.name)
    )
    ...
    const inviteeIdList = inviteeList.length
        ? inviteeList.map((i: any) => i.username)
        : inviteeList.username
...
export function getNickName(linkList: any, name: string) {
    ...
    const inviteeIdList = inviteeList.length
        ? inviteeList.map((i: any) => i.nickname)
        : inviteeList.nickname
    return inviteeIdList
}
