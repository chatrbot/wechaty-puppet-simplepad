...
export const roomJoinParser = async (
    puppet: Puppet,
    message: Message
    ...
    let content = clearEscapeFlag(contents.join(':'))

    const jsonPayload = await xmlToJson(content)
    if (
        !jsonPayload ||
        !jsonPayload.sysmsg ||
        !jsonPayload.sysmsg.sysmsgtemplate
    ) {
        return null
    }
    ...
    const linkList =
        jsonPayload.sysmsg.sysmsgtemplate.content_template.link_list.link
    ...
    const matchesForBotInviteOther =
        matchesForBotInviteOtherEn || matchesForBotInviteOtherZh
    const matchesForOtherInviteBot =
        matchesForOtherInviteBotEn || matchesForOtherInviteBotZh
    const matchesForOtherInviteOther =
        matchesForOtherInviteOtherEn || matchesForOtherInviteOtherZh
    const matchesForOtherInviteOtherQrcode =
        matchesForOtherInviteOtherQrcodeEn || matchesForOtherInviteOtherQrcodeZh
