...
class PuppetSimplePad extends Puppet {
      ...
      if (typeof recv === "string") {
        ...
        try {
          ...
          if (recvData.data.reportMsgType === ReportType.Message) {
            ...
            const { category, payload } = await parseMessage(this, msg);
            switch (category) {
              ...
              case MessageCategory.Friendship:
                this._cacheMgr?.setFriendshipRawPayload(messageId, payload);
                this.emit("friendship", {
                  friendshipId: messageId,
                });
                break;
              case MessageCategory.RoomInvite:
                this._cacheMgr?.setRoomInvitation(messageId, payload);
          ...
          // 接收到的群信息变更消息,更新群信息
          if (recvData.data.reportMsgType === ReportType.ChatroomNotify) {
            const notify = (recvData as ReceiveData<ChatroomNotify>).data;
            const room = await this._cacheMgr?.getRoom(notify.userName);
            if (room) {
              ...
              if (room.chatroomVersion != notify.chatroomVersion) {
                room.chatroomVersion = notify.chatroomVersion;

                const members = await this._cacheMgr!.getRoomMember(
                  notify.userName
                );
                if (members) {
                  // 有新成员加入
                  if (
                    notify.chatroomMembers.length > Object.keys(members).length
                  ) {
                    const newMembers =
                    ...
                    this._cacheMgr?.setRoomMember(notify.userName, members);
                  }
                  ...
                  if (
                    ...
                    notify.chatroomMembers.map((m) => {
                      const oldMember = members[m.userName];
                      if (oldMember) {
                        newMembers[oldMember.userName] = oldMember;
                      }
                    });
                    this._cacheMgr?.setRoomMember(notify.userName, newMembers);
                  }
                }
              }
  ...
  protected async contactRawPayload(id: string): Promise<Contact | undefined> {
  ...
  async messageFile(messageId: string): Promise<FileBox> {
    ...
    switch (messagePayload.type) {
      case MessageType.Image:
        const imgUrl = await this._client?.DownloadImage(messagePayload.text!);
  ...
  async messageSendContact(
    ...
    const rsp = await this._client.SendPersonalCardMessage(toUser, contactId);
  ...
  async messageSendMiniProgram(
    ...
    const rsp = await this._client.SendMiniProgram(toUser, miniProgramPayload);
    await this._setSendMessage(SimplePadMessageType.App, toUser, rsp);
    await this._setMessageRevokeInfo(toUser, rsp);
    return rsp.newMsgId;
  }
  ...
  async messageSendText(
    ...
    const rsp = await this._client.SendTextMessage(toUser, text, mentionIdList);
  ...
  async roomAnnounce(roomId: string, text?: string): Promise<string | void> {
    ...
    const extRoomInfo = await this._client.GetChatroomExtraInfo(room.userName);
    if (extRoomInfo) {
  ...
  protected async roomRawPayload(roomId: string): Promise<Contact | undefined> {
  ...
  async tagContactAdd(tagName: string, contactId: string): Promise<void> {
    ...
    await this._client?.EditContactLabel(contactId, contactLabelIds.join(","));
  ...
  // 暂时不支持设置联系人电话功能
  async contactPhone(_contactId: string, _phoneList: string[]): Promise<void> {
    throw new Error(`not implement`);
  }
  ...
  private async _updateContactCache(contact: Contact): Promise<void> {
    ...
    if (isRoomId(contact.userName)) {
      ...
      if (!members || Object.keys(members).length == 0) {
        // 初始化群成员信息
        const detail = await this._client.GetChatroomMemberDetail(roomId);
        if (detail) {
          ...
          await detail.memberList.map(async (member) => {
            ...
            if (!(await this._cacheMgr?.hasContact(member.userName))) {
              const _contact = this.parseMemberToContact(roomId, member);
              await this._cacheMgr?.setContact(_contact.userName, _contact);
              await this.dirtyPayload(PayloadType.Contact, _contact.userName);
            }
          });
